...
export const Config = z.object({
  ...
  providerPriceUrls: z.string().default('{}'),
  providerModes: z.string().default('{}'),
...
const viewSchema = zod.object({
  ...
  providerPrices: zod.array(zod.object({
    provider: zod.string(),
    model: zod.string(),
    input: zod.number().nonnegative(),
    output: zod.number().nonnegative(),
...
export function apply(ctx, config = {}) {
  ...
  /** 把 after 表中覆盖的模型合并到 base 表（after 优先，跳过 overrides）。 */
  const mergeProviderFlat = (base, after, spec) => {
    for (const [m, e] of Object.entries(after)) {
      if (spec.overrides[m] !== undefined) continue
      base[m] = e
    }
  }

  /**
   * 为每个 provider 装配价格表（重建投影前调用）：
   *  - pi-ai 本地价自动兜底（DSH 已知的所有云 provider，无需配置 URL；source='pi-ai'）；
   *  - 磁盘缓存里的 URL 抓取结果（providerFlat）覆盖之（source='custom-json'）。
   */
  const attachProviderPrices = (spec) => {
    if (!spec.providerPrices) spec.providerPrices = {}
    if (!spec.providerSources) spec.providerSources = {}
    // 1) pi-ai 本地：为 DSH 已知的所有云 provider 自动兜底
    for (const provider of listPiAiProviders()) {
      applyProviderPrices(spec, provider, loadPiAiProvider(provider))
    }
    // 2) 缓存中的 URL 抓取结果覆盖（配了 providerPriceUrls 的 provider）
    const cached = lastFetched ?? loadPriceCache(cachePath)
    if (cached !== null && cached.providerFlat && typeof cached.providerFlat === 'object') {
      for (const [provider, flat] of Object.entries(cached.providerFlat)) {
        if (typeof flat !== 'object' || flat === null) continue
        const bucket = (spec.providerPrices[provider] ??= {})
        mergeProviderFlat(bucket, flat, spec)
  ...
  const registerProjection = (spec) => {
    ...
    // 兼容双核心（0.1.0-rc.8 读顶层 schema/view；0.1.1-rc.1+ 只读 wire 块——
    // 缺 wire 时注册表视单元为 host-only：快照跳过、帧不推送、onChanged 不通知）。
    // stateSchema 校验的是持久化检查点里的单元状态（纯 JSON 契约），不是视图。
    disposeProjection = ctx.sessionProjections.register({
      key: parts.key,
      stateSchema: zod.json(),
      init: parts.init,
      apply: parts.apply,
      view: parts.view,
      wire: { viewSchema, view: parts.view },
  ...
  const refreshPrices = async (force = false) => {
    ...
    const cfg = current()
    let providerUrls = {}
    try { providerUrls = parseProviderUrlMap(cfg.providerPriceUrls) } catch { providerUrls = {} }
    const cached = loadPriceCache(cachePath)
    const urlsEqual = cached !== null && cached.providerUrls !== undefined
      && JSON.stringify(cached.providerUrls) === JSON.stringify(providerUrls)
    const stale = cached === null
      ...
      || (Object.keys(providerUrls).length > 0 && !urlsEqual)
    ...
    try {
      ...
      // 按 provider 抓自定义 URL（各自独立容错，不阻塞整体）；折算后存入缓存 providerFlat
      const providerFlat = {}
      if (Object.keys(providerUrls).length > 0) {
        const spec = resolveBillingSpec(cfg)
        for (const [provider, url] of Object.entries(providerUrls)) {
          try {
            const pf = await fetchJsonPrices(url)
            applyFetchedPrices(spec, pf, Date.now(), provider)
            if (spec.providerPrices[provider]) providerFlat[provider] = spec.providerPrices[provider]
          } catch (err) {
            console.warn(`[token-billing] provider「${provider}」价格抓取失败：${err.message}`)
          }
        }
      }
      if (isBuiltin && Object.keys(providerUrls).length === 0) {
        // builtin 且无 provider URL：无抓取记录，但可展示内置表当前生效价
  ...
  /** 账本里实际用过（有计费 step）的 provider 集合；空则回退显示全部。 */
  const usedProvidersOf = () => {
    const set = new Set()
    for (const s of ledger.steps ?? []) {
      if (s.provider && s.provider !== '' && s.provider !== '(无)') set.add(s.provider)
    }
    return set
  }

  /** 按 provider 的当前生效单价表（已折算，按 now 计价），供状态页展示 provider 模型价格。 */
  const providerEffectiveFrom = (now = Date.now()) => {
    if (currentSpec === null) return {}
    const used = usedProvidersOf()
    const out = {}
    for (const [provider, bucket] of Object.entries(currentSpec.providerPrices ?? {})) {
      if (used.size > 0 && !used.has(provider)) continue
      const prov = {}
      for (const [model, entry] of Object.entries(bucket)) {
        prov[model] = {
          ...
          currency: entry.currency ?? currentSpec.currency,
        }
      }
      out[provider] = prov
    }
  ...
  const statusWithEffective = (record, now = Date.now()) => {
    ...
    if (record === null || currentSpec === null) return { ...base, effective: {}, providerEffective: {} }
    return { ...base, effective: effectiveFrom(now), providerEffective: providerEffectiveFrom(now) }
  }
  ...
  if (typeof ctx.inject === 'function') {
    ctx.inject(['webServer'], (host) => {
      ...
      /** 历史统计（今日/本月/累计/按模型/按天/节省/收费形式/缓存命中）。 */
      host.webServer.register({
        ...
        handler: (req, res) => {
          const cfg = current()
          let providerModes = []
          try { providerModes = parseProviderModes(cfg.providerModes) } catch { providerModes = [] }
          ...
          // 图表辅助数据：近 14 天本地时区时间轴 + 月度预算（供统计页折线图/预算环使用）
          const tzNow = Date.now()
          const tzKey = cfg.offPeakTz || 'Asia/Shanghai'
          const seriesLabels = []
          for (let i = 13; i >= 0; i -= 1) seriesLabels.push(dayKeyOf(tzNow - i * 86_400_000, tzKey))
          stats.seriesLabels = seriesLabels
          stats.budget = {
            monthly: Number.isFinite(cfg.monthlyBudget) && cfg.monthlyBudget > 0 ? cfg.monthlyBudget : 0,
            currency: cfg.currency || '¥',
          }
          res.writeHead(200, { 'content-type': 'application/json' })
