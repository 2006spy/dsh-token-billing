...
/**
 * 某时刻某模型的生效价格（含高峰/错峰）。
 * 用户覆盖（overrides）不参与高峰/错峰 —— 覆盖即钉死。
 */
export function priceAt(model, spec, atMs) {
  const key = modelKey(model)
 ...
 */
export function applyFetchedPrices(spec, fetched, now = Date.now()) {
...
function settleStep(state, active, spec, atMs) {
  ...
  const price = priceAt(model, spec, atMs)
...
function refundLastEstimated(state, spec) {
  ...
  const buckets = step === undefined ? undefined : {
    uncachedInputTokens: step.uncachedInputTokens,
  ...
  if (buckets !== undefined) {
    ...
    subCost(state.totals.costs, last.currency, last.cost)
...
export function createBillingView(spec) {
  return function view(state) {
    ...
    const activePrice = active === null ? null : priceAt(active.model, spec, atMs)
