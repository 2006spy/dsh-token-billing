...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function BillingSettingsCard(props) {
      ...
      // 命中「统计」页：历史账本（今日/本月/累计/按模型/按天）+ 余额 + 节省 + 导出
      if (section === '统计') {
        ...
        // 汇总卡
        var costOf = function (obj) { return formatCosts((obj && obj.costs) || {}, '¥') }
        var callsOf = function (obj) { return obj ? obj.calls : 0 }
        var summary = [
          { label: '今日', value: stats ? costOf(stats.today) : '—', sub: stats ? callsOf(stats.today) + ' 次调用' : '' },
          { label: '本月', value: stats ? costOf(stats.month) : '—', sub: stats ? callsOf(stats.month) + ' 次调用' : '' },
          { label: '累计', value: stats ? costOf(stats.total) : '—', sub: stats ? callsOf(stats.total) + ' 次调用' : '' },
        ]
        nodes.push(h('div', { key: 'st-summary', style: ST_BLOCK },
          h('label', { style: Object.assign({}, CARD_CSS.label, { display: 'block', marginBottom: '2px' }) }, '费用汇总'),
          h('div', { style: ST_GRID },
            summary.map(function (s) {
              return h('div', { key: s.label, style: ST_CARD },
            ...
            (function () {
              ...
              return names.map(function (m) {
                ...
                return h('div', { key: m, style: ST_ROW },
                  ...
                  h('span', { style: ST_CELL_NUM }, formatCosts(entry.costs, '¥') + ' · ' + entry.calls + ' 次'),
                )
              })
            })(),
          ),
