...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 从模型选择器读取当前选中的模型显示名（如 "DeepSeek-V4-Pro"）。 */
    ...
    /**
     * 可视化模型价格编辑器。
     * @param props.value - prices JSON 字符串（受控，来自 draft）。
     * @param props.onChange - (newJsonString) => void。
     */
    function ModelPriceEditor(props) {
      var value = props.value
      var disabled = props.disabled
      var lastValid = useRef(null)
      var parsed = parsePricesRows(value)
      var rows, error
      if (parsed.ok) {
        lastValid.current = parsed.rows
        rows = parsed.rows
        error = null
      } else {
        rows = lastValid.current || []
        error = parsed.error
      }
      ...
      function updateRow(index, field, next) {
        ...
        props.onChange(serializePricesRows(copy))
      }
      function addRow() {
        props.onChange(serializePricesRows(rows.concat([emptyPriceRow()])))
      }
    ...
    function BillingSettingsCard(props) {
      ...
      useEffect(function () {
        ...
        setDraft(function (prev) {
          ...
          for (var i = 0; i < FIELDS.length; i++) {
            ...
            if (t[f.key]) {
              if (prev[f.key] === undefined) { prev[f.key] = resolved; changed = true }
              continue
            }
            if (prev[f.key] !== resolved) { prev[f.key] = resolved; changed = true }
          }
          return changed ? Object.assign({}, prev) : prev
        })
      }, [ready, value])
      ...
      return h('li', { style: cardStyle },
        header,
        open ? h('div', { style: CARD_CSS.body },
